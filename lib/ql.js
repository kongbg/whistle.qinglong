...
const queue = async.queue((task, options, completed) => {
    console.log("Currently Busy Processing Task " + task);
    methods[options.fnkey](options).then((result) => {
        if (result.isOk) {
            queue.kill();
...
function handleKs(req, res, storage, rule) {
...
function handleHemi(req, res, storage, rule) {
...
function handle(req, res, storage) {
    let cookieObj = storage.getProperty('cookieObj');
